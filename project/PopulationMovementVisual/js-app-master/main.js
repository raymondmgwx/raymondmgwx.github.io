    ...
    render: {
        value: function() {
            return $("<a>", {
                ...
                [
                    $("<span>", { class: "selector-icon icon" }), $("<span>", { class: "selector-content" }).append([
                        $("<span>", { class: "selector-subtitle" }), $("<span>", { class: "selector-text" })
                    ])
                ]
            );
        }
            ...
            return $("<div>", { class: "modal" }).append(
                $("<div>", { class: "modal-inner" }).append([
                    $("<header>", { class: "modal-header" }).append(
                        ...
                        $("<div>", { class: "list" })), $("<footer>", { class: "modal-footer tool-bar" }).append([
...
table_view.prototype = Object.create(viewer.prototype, {
    ...
    draw: {
        value: function(data) {
            ...
            if (!dat.canDraw) return;
            var thead_tr = [
                dat.metadata[dat.axis].name, "値", "単位"
            ]
            if (_this.index == 0)
                thead_tr.unshift('表示')
...
App.prototype = {
    ...
    init: function() {
        ...
        this._chart_typeDialog.setItems([
            { text: "棒グラフ", icon: "bar-chart", chart_type: Data.CHART_TYPE_BAR_CHART }, { text: "折れ線グラフ", icon: "line-chart", chart_type: Data.CHART_TYPE_LINE_CHART }, { text: "散布図", icon: "scatter-plot", chart_type: Data.CHART_TYPE_SCATTER_PLOT }, { text: "地図", icon: "map-plot", chart_type: Data.CHART_TYPE_MAP }
        ]);
        ...
        this._tabbar = new side_bar("side-bar-tab-bar", [
            { text: "X", element: this.dataPanel1.element }, { text: "Y", element: this.dataPanel2.element }
        ]);
...
initialization.data_load = function() {
    var app = new App();
    app.loadAppId();
    var appId = "22354c16f08ca03a198de07844782f5f47f5f59c";
    try {
        window.localStorage.setItem(App.KEY_APP_ID, appId);
    } catch (e) {
        document.cookie = 'appId=' + appId
