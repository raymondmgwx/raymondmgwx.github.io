...
//restoring force param
var fulcrum = { x: 0, y: 0, z: 300 };
var k = 1.0;
...
Ball.prototype = {
    ...
    calculateForce: function() {
        ...
        var fx = -k * (this.x - fulcrum.x);
        var fy = -k * (this.y - fulcrum.y);
        var fz = -k * (this.z - fulcrum.z) - this.mass * g;
    ...
    calculateEnergy: function() {
        ...
        var l2 = Math.pow(this.x_1 - fulcrum.x, 2) + Math.pow(this.y_1 - fulcrum.y, 2) + Math.pow(this.z_1 - fulcrum.z, 2);
        var kinetic = 1 / 2 * this.mass * v2;

        var potential = this.mass * g * this.z_1 + 1 / 2 * k * l2;
...
function initObject() {
    ...
    scene.add(cube);
    cube.position.set(fulcrum.x, fulcrum.y, fulcrum.z);
...
function update_param() {
    ...
    geometry.vertices[0] = new THREE.Vector3(fulcrum.x, fulcrum.y, fulcrum.z);
    geometry.vertices[1] = new THREE.Vector3(ball.x, ball.y, ball.z);
